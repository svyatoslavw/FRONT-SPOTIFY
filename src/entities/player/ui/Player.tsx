...
const Player: FC<TrackProps> = ({ activeId, track, trackUrl }) => {
  const { setId, setVolume, setIsPlaying, volume, isPlaying, ids } =
    usePlayer();
  ...
  return (
    <div className="h-20 w-full justify-between fixed bottom-0 flex items-center px-3 bg-gradient-to-t from-black via-zinc-950 to-zinc-950">
      {track ? (
        <div className="flex gap-2 w-72 items-center p-2 my-2 rounded-md ">
          ...
          <div className="w-full">
            <p className="text-sm w-64 truncate cursor-pointer hover:underline">
              {track.name}
            </p>
